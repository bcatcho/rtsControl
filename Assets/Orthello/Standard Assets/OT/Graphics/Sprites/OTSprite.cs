...
public class OTSprite : OTObject
{
	...
	private void SetMatReference()
	{
		...
		else if (_materialReference == "additive" || _materialReference == "transparent" || _materialReference == "")
	...
	override protected void CheckDirty()
	{
		...
		else if (_spriteContainer_ != null || image != _image_)
			isDirty = true;
		...
		else if (additive != _additive_ && additive) {
			_transparent = false;
			_additive_ = additive;
			_transparent_ = transparent;
			SetMatReference();
		}
		else if (!_additive && !_transparent) {
			_additive_ = additive;
			_transparent_ = transparent;
		...
		if (materialReference != _materialReference_) {
			...
			else if (_materialReference == "additive") {
				_transparent = false;
				_additive = true;
			}
			else {
					...
					if ((spriteContainer as OTSpriteAtlas).offsetSizing) {
						...
						if (_frameIndex_ != frameIndex || _spriteContainer_ != spriteContainer) {
							...
							if (objectParent && !Vector3.Equals(size, sc))
								size = sc;
							else if (!Vector3.Equals(transform.localScale, sc3))
